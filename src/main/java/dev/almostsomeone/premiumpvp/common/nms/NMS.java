...
public class NMS {
    ...
    public NMS(final Plugin plugin) {
        ...
        this.plugin.getLogger().log(Level.INFO, "Hooking into NMS " + version);

        this.globalClassBuilder = new GlobalClassBuilder();
        try {
            globalClassBuilder.init(version, plugin);
        } catch (ClassNotFoundException exception) {
            this.plugin.getLogger().log(Level.WARNING, "Failed to hook into NMS " + version + ". Please check if you are running a supported fork, if you are, contact the developer.");
